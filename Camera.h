...
namespace camera
{
    ...
    public:
        ...
        void RotateFirstPerson_OX(float angle)
        {
            ...
            forward = normalize(glm::normalize(forward));
        ...
        void SetPosition(Tank *tank, float rotationAngle)
        {
           ...
           // printf("%f %f %f\n", firstX, firstY, firstZ);
            float x = tank->getCorp()->getX() - firstZ * sin(angle);
            float y = 0;
            float z = tank->getCorp()->getZ() - (- firstZ) * cos(angle);

           position = glm::vec3(x, firstY, z);
           RotateThirdPerson_OY(rotationAngle);
        }
